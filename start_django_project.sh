...
cat $PUBHOSTFILE | PUBHOSTURL="$PUBHOSTURL" jq 'map(if .PUBHOST_URL == "127.0.0.1:443" then . + {"PUBHOST_URL":env.PUBHOSTURL} else . end)' > $PUBHOSTTMP && mv $PUBHOSTTMP $PUBHOSTFILE
