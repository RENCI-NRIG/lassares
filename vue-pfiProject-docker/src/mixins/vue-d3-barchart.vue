...
<svg class="d3-bar-chart" v-if="bars.length" :width="w" :height="h">
  <defs v-if="renderGradient">
    <lineargradient :id="gradientId" x1="0" x2="100%" y1="0" y2="0">
      <stop v-for="d,i in bars" :offset="d.percentX + &quot;%&quot;" :key="i" :stop-color="d.color"></stop>
    </lineargradient>
  </defs>
  <g class="lines" v-if="opts.axis.linesY">
    <line class="line-y" v-for="a,i in axisY" :x1="margin" :x2="w" :y1="a.y" :y2="a.y"></line>
  </g>
  <g class="axis" v-if="opts.axis">
    <line class="x-axis" :x1="oX" :x2="w" :y1="hh" :y2="hh"></line>
    <line class="y-axis" :x1="oX" :x2="oX " y1="0" :y2="hh"></line>
    <g class="axis-labels">
      <template v-for="a,i in axisY"></template>
      <text class="axis-label" v-if="opts.axis.valuesY" x="0" :y="a.y">{{a.value}}</text>
    </g>
  </g>
  <g class="curve-back" v-if="opts.curveBack">
    <path :d="curve(opts.curveBack)" :stroke="curveBackStyle.stroke" :style="curveBackStyle" :fill="curveBackStyle.fill"></path>
  </g>
  <g class="bars">
    <template v-for="d,i in bars">
      <rect class="bar" v-if="opts.bars" :width="d.w" :height="d.y" :x="barX(d)" :y="barY(d)" :style="barStyle(d)" @click="barClick($event,d)"></rect>
    </template>
  </g>
  <!-- <g class="curve" v-if="opts.curve">
    <path :d="curve(opts.curve)" :stroke="curveStyle.stroke" :style="curveStyle" :fill="curveStyle.fill"></path>
  </g> -->
  <g class="dummies">
    <template v-for="d,i in bars">
      <rect class="dummy-bar" v-if="(opts.tip || opts.line)" :width="d.w" :height="h" :x="barX(d)" y="0" :class="(opts.bars) ? &quot;has-bars&quot;:&quot;&quot;" @mouseover.prevent="startMove($event,d)" @mouseleave="stopMove($event,d)" @click="barClick($event,d)" @touchstart="barClick($event,d)"></rect>
    </template>
  </g>
  <g class="marks" v-if="opts.marks">
    <rect class="mark" v-if="opts.marks.type === &quot;square&quot;" v-for="d,i in bars" :key="i" :x="barX(d) + (barW /2) - (markSize /2)" :y="barY(d)-(markSize /2)" :width="markSize" :height="markSize" :style="markStyle(d)"></rect>
    <circle class="mark" v-if="opts.marks.type !==&quot;square&quot;" v-for="d,i in bars" :key="i" :r="markSize / 2" :cx="barX(d) + barW /2" :cy="barY(d)" :style="markStyle(d)"></circle>
  </g>
  <g class="chart-line" v-if="opts.line" v-show="over">
